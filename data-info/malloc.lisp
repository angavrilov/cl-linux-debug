                   ...
                   (multiple-value-bind (id offset)
                     ...
                     (if id
                         (unless (or (/= 0 offset) (aref reftbl id))
                           (setf ref (cons addr (cdr type)))
                           (setf (aref reftbl id) (car type)))
                         (unless (gethash addr hash)
                           (setf ref (cons addr (cdr type)))
                           (setf (gethash addr hash) (car type))))
                     (when (and ref (cdr ref))
                       (chanl::pushend ref queue-head queue-tail))
                     nil))
                 (walk-ref (item)
                   (funcall (the function (cdr item)) memory-cb (car item) #'queue-ref)
                   #+nil(walk-reference ref #'queue-ref)
                   nil
                   #+nil
          ...
          (dolist (global *known-globals*)
            (let ((ref ($ memory (car global))))
              (queue-ref (memory-object-ref-address ref)
                         (get-effective-pointer-walker memory (memory-object-ref-type ref))
                         nil nil)))
